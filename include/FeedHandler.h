...
template <typename T, size_t CAPACITY> struct Pool {
  T *storage;
  uint32_t head;

  Pool() {
    storage = static_cast<T *>(
        mmap(nullptr, CAPACITY * sizeof(T), PROT_READ | PROT_WRITE,
...
public:
  ...
  void processOrder(uint64_t &id, databento::Side &side, uint32_t instrument_id,
                    databento::Action &action, int64_t &price, uint32_t &qty,
                    databento::UnixNanos &ts_recv,
                    databento::TimeDeltaNanos &ts_event);

  void match(Order &entry, databento::Side &side, int64_t &price,
             uint32_t instrument_id);
  bool modifyOrder(uint64_t order_id, int64_t new_price, uint32_t new_qty,
                   uint32_t instrument_id);

  void trade(uint64_t id, databento::Side &side, int64_t &price, uint32_t qty,
             uint32_t instrument_id);
  void addOrder(Order &entry, databento::Side &side, int64_t &price,
                uint32_t instrument_id);
  ...
  void prep(const std::basic_string<char> &symbol, int64_t low, int64_t high,
            int64_t tick_size) {
    SymbolBook book;
  ...
  void reserveBooks(size_t count) { books.reserve(count); }

  void finalizePrep() {
    std::sort(books.begin(), books.end());
    // for (auto& [id, sym] : id_to_symbol_) {
    for (auto id = 0; id < MAX_INSTRUMENT_ID; id++) {
      // if (id < MAX_INSTRUMENT_ID)
      auto sym = id_to_symbol_[id];
